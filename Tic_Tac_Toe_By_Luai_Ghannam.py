...
def check_for_winner(player):
    for i in range(3):
        ...
        if player['column{} score'.format(i)] == (3 * player['sprite weight']):
            return True   
#--------------------------------------------------------------------------------------------------------------------#
    if player['diagonal1 score'] == (3 * player['sprite weight']):
        return True
    if player['diagonal2 score'] == (3 * player['sprite weight']):
        return True
    return False    
#--------------------------------------------------reset_scores------------------------------------------------------#
'''                         reset_scores is called within score_claculator function.
    ...
    By using this function:
        ...
        X|2|3                              X|2|3
        -----                              -----
        4|5|6  here, diagonal score = 10,  4|X|6 now diagonal score = old diagonal score(0) + new diagonal score(20) = 20 
        -----                              ----- still not enough to win.
        7|8|9                              7|8|9
''' 
def reset_scores(player,size):
    for i in range(size):
        player['row{} score'.format(i)] = 0
        player['column{} score'.format(i)] = 0
    player['diagonal1 score'] = 0
    player['diagonal2 score'] = 0
    return player
#---------------------------------------score_calculator---------------------------------------------------------#
...
def player_selection():
    ...
    number = 0
    while (number > 9 or number < 1) or not player_input.isdigit():
...
def input_decoding(game_placement,sprite):
    ...
    while not correct_input:
        ...
        i = (player_input - 1) // 3   ##floor division##
        j = (player_input - 1) %  3 ##Saw tooth function##
...
def who_plays_first(player1_name,player2_name):
    ...
    # Use a loop to keep rolling until a tie is statistically impossible
    while throw_dictionary[player1_name] == throw_dictionary[player2_name]:
        throw_dictionary[player1_name] = random_dice()
        throw_dictionary[player2_name] = random_dice()
...
def initialize_new_game(player1, player2):
    ...
    # 1. Create independent memory clones immediately using original names
    player1_copy = copy.deepcopy(player1)
    player2_copy = copy.deepcopy(player2)
    
    player_choice = who_plays_first(player1_copy['name'], player2_copy['name'])
    player_sprite = choose_between_X_and_O(player_choice, player1_copy['name'], player2_copy['name'])
    
    player1_copy['sprite'] = player_sprite[player1_copy['name']]
    player2_copy['sprite'] = player_sprite[player2_copy['name']]

    player1_copy['sprite weight'] = sprite_weight(player1_copy['sprite'])
    player2_copy['sprite weight'] = sprite_weight(player2_copy['sprite'])

    player1_copy = reset_scores(player1_copy, 3)
    player2_copy = reset_scores(player2_copy, 3)
    player1_copy['winner'] = False
    player2_copy['winner'] = False
        
    initialize['player1'] = player1_copy
    initialize['player2'] = player2_copy
